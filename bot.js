...
const random = {
  int: function(a, b) { a = Math.round(a); b = Math.floor(b); return Math.floor(Math.random() * (b - a + 1)) + a },
...
client.on("messageCreate", msg => {
  if (!msg.author.bot) {
    if (/\$_facts\s+[0-9]+/.test(msg.content)) {
      ...
      async function f() {
        channels['b' + msg.channel.id] = 0
        for (let i = 0; i < num; i++) {
          await sleep(1000)
          const embed = new Discord.EmbedBuilder()
            .setColor([random.int(100, 255), random.int(100, 255), random.int(100, 255)])
            .setTitle(`Fact ${i + 1}:`)
            .setDescription(`${ret[i]}`)
            .setFooter({ text: 'Did You Like The Fact' });
          //`Fact ${i+1} : \n-> ${ret[i]}`
          let react = msg.channel.send({ embeds: [embed] })
          react.then(m => m.react('👍'))
          react.then(m => m.react('👎'))
        }
        delete channels['b' + msg.channel.id]
      }
      if (channels.hasOwnProperty('b' + msg.channel.id)) {
        let error = new Discord.EmbedBuilder()
          .setColor([255, 0, 0])
          .setTitle(`ERROR :`)
          .setDescription(`An Error Occured! Feels like someone is viewing facts here.`)
        msg.author.send({ embeds: [error] })
      }
      else { f() }
